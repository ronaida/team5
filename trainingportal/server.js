...
var adminPanelHtml = fs.readFileSync(path.join(__dirname, 'static/adminPanel.html'),'utf8');
var InstructorHtml = fs.readFileSync(path.join(__dirname, 'static/InstructorMain.html'),'utf8');
...
function(req, res) {
  ...
  var userid = "Local_" + username;
  console.log(userid);
  db.getUserByIdd(userid, null, async (user) => {
    if(user){
        res.redirect(handleUserNavigation(user));
    }
    });
});
function handleUserNavigation(user)
{ 
  console.log("das");
    if(user[0].isAdmin==0&&user[0].IsInstructor==1)
    {
      return "/instructor";
    }
...
app.get('/adminpanel', (req, res) => {
  ...
  if(req.user.isAdmin==1){
    let updatedHtml = auth.addCsrfToken(req, adminPanelHtml);
    res.send(updatedHtml);
  

  }
  else{
    res.send("you don't have enough access");
...
app.get('/instructor',async (req, res) => {
  if(req.user.IsInstructor==1){
    let updatedHtml = auth.addCsrfToken(req, InstructorHtml);
...
app.get('/challenges/solutions/:challengeId', (req,res) => {
  if(req.user.InstructorID!=0 && req.user.IsInstructor!=1){
    var aa=true;
    db.checkblockedchallenge(req.user.InstructorID,req.params.challengeId, null, async (allowed) => {
      if(allowed.length>0){
        aa=false;
        res.send("You are not allowed to see this solution");
...
app.post('/getremoved', async (req, res) => {
  db.checkblockedinstructor(req.user.id, null, async (allowed) => {
      res.send(allowed);

    });
});
app.post('/getremovedstudent', async (req, res) => {
  db.checkblockedinstructor(req.user.InstructorID, null, async (allowed) => {
      res.send(allowed);

    });
});
app.post('/removedsolution', async (req, res) => {
  console.log("saww");
  console.log(req.body.challenge);
  db.togglesolution(req.user.id,req.body.challenge);

...
exports.registerLocalUser = function(req,res){
    ...
    // var Isinstructor = newUser.IsInstructor;
    var Isinstructor = false;

    var InstructorU = newUser.InstructorU;
    console.log(InstructorU);
    ...
    if(!Isinstructor){
        ...
        if(!(InstructorU in localUsers)){
            return util.apiResponse(req, res, 400, "Invalid Instructor username.");
        }
    }
    ...
    exports.createUpdateUser(req, res, username, localUser, password,Isinstructor,InstructorU);
    
};


exports.createUpdateUserInternal = (username, localUser, password,Isinstructor,InstructorU) => {
  //create user
  ...
  localUser.Isinstructor = Isinstructor.toString();
  localUser.InstructorU = InstructorU.toString();
...
exports.createUpdateUser = function(req, res, username, localUser, password,Isinstructor,InstructorU){
...
processAuthCallback = function (profileId, givenName, familyName, email, cb,IsInstructor,InstructorU) {
    ...
    if(user){
        ...
        user = {
            ...
            givenName: givenName,
            IsInstructor: 0,
            InstructorID: InstructorU,
            teamId: null,
            level:0
        };

        db.getinstructorID(InstructorU, null, async (InstructUn) => {
            user.InstructorID = InstructUn[0]["id"];
            // console.log(InstructUn);
...
getLocalStrategy = function () {
    return new LocalStrategy((username, password, cb) => {
        ...
        if(user!==null){
            return processAuthCallback("Local_"+username, user.givenName, user.familyName, null, cb,user.Isinstructor,user.InstructorU);
        }

...
exports.insertUser = function(user,errCb,doneCb){
  var con = getConn();
  var sql = "INSERT INTO users (id, accountId, teamId, familyName, givenName,IsInstructor,InstructorID) VALUES (null, ?, null, ?, ?,?,?)";
  con.query(sql, [user.accountId, user.familyName, user.givenName,user.IsInstructor,user.InstructorID], function (err, result) {
    if (err) handleDone(doneCb,1);
...
exports.checkblockedchallenge = function(accountId,challengeid,errCb,doneCb){
  console.log(accountId);
  console.log(challengeid);

  var con = getConn();
  var sql = "select * FROM hidden_challenges WHERE Insturctor_ID = ? and Challenge_ID= ? ";
  con.query(sql, [accountId,challengeid], function (err, result) {
    if(err) handleErr(errCb,err);
    else handleDone(doneCb,result);
  });
};
exports.checkblockedinstructor = function(accountId,errCb,doneCb){
  // console.log(accountId);
  // console.log(challengeid);

  var con = getConn();
  var sql = "select * FROM hidden_challenges WHERE Insturctor_ID = ?";
  con.query(sql, [accountId], function (err, result) {
    if(err) handleErr(errCb,err);
    else handleDone(doneCb,result);
  });
...
exports.getinstructorID = function(accountId,errCb,doneCb){
  ...
  var con = getConn();
  var newaccwithlocal="Local_"+accountId;
  var sql = "select * FROM users WHERE accountId = ? or  accountId = ? ";
  con.query(sql, [accountId,newaccwithlocal], function (err, result) {
    if(err) handleErr(errCb,err);
    else handleDone(doneCb,result);
  });
};
exports.togglesolution = function(accountId,challengeid){
  var con = getConn();
  var sql = "select * FROM hidden_challenges WHERE Insturctor_ID = ?  and Challenge_ID= ? ";
  con.query(sql, [accountId,challengeid], function (err, result) {
    ...
    if(result.length>0){
      var conw = getConn();
      var sqlw = "delete FROM hidden_challenges WHERE Insturctor_ID = ?  and Challenge_ID= ? ";
      conw.query(sqlw, [accountId,challengeid], function (err, result) {
      });

    }
    else{
      var conw = getConn();
      var sqlw = "INSERT INTO hidden_challenges (Insturctor_ID, Challenge_ID) VALUES (?, ?);";
      conw.query(sqlw, [accountId,challengeid], function (err, result) {
      });
